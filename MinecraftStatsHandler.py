...
username_map = {}
# Load config for ignore rules (optional)
try:
    with open('StatCrafterConfig.json', 'r') as _cf:
        _CFG = json.load(_cf)
except Exception:
    _CFG = {}
    ...
    def get_minecraft_usernames(self):
        try:
            ...
            if response.status_code == 200:
                ...
                # Check ignore rules from config
                prefixes = _CFG.get("ignore_name_prefixes", [])
                suffixes = _CFG.get("ignore_name_suffixes", [])
                ...
                if is_ignored(self.username):
                    # Do not add ignored names to the username map and signal that this user should be skipped
                    return f"Ignored username: {self.username}"
                ...
                if config_data.get("offline_supported", True):
